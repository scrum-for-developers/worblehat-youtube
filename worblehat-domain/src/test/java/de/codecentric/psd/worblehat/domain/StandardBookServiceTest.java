...
public class StandardBookServiceTest {
	...
	@Before
	public void setup() {
		aBook = new Book(new BookParameter("title", "author", "edition", "isbn", 2016));
		aCopyofBook = new Book(new BookParameter("title", "author", "edition", "isbn", 2016));
		anotherBook = new Book(new BookParameter("title2", "author2", "edition2", "isbn2", 2016));

		aBorrowedBook = new Book(new BookParameter("title", "author", "edition", "isbn", 2016));
		aBorrowing = new Borrowing(aBorrowedBook, BORROWER_EMAIL, NOW);
		aBorrowedBook.borrowNowByBorrower(BORROWER_EMAIL);

		aCopyofBorrowedBook = new Book(new BookParameter("title", "author", "edition", "isbn", 2016));
		...
		anotherBorrowedBook = new Book(new BookParameter("title2", "author2", "edition2", "isbn2", 2016));
	...
	public void shouldCreateBook() {
		...
		bookService.createBook(
				new BookParameter(aBook.getTitle(), aBook.getAuthor(), aBook.getEdition(), aBook.getIsbn(), aBook.getYearOfPublication()));
	...
	public void shouldNotCreateAnotherCopyOfExistingBookWithDifferentTitle() {
		givenALibraryWith(aBook);
		bookService.createBook(
				new BookParameter(aBook.getTitle() + "X", aBook.getAuthor(), aBook.getEdition(), aBook.getIsbn(), aBook.getYearOfPublication()));
	...
	public void shouldNotCreateAnotherCopyOfExistingBookWithDifferentAuthor() {
		givenALibraryWith(aBook);
		bookService.createBook(
				new BookParameter(aBook.getTitle(), aBook.getAuthor() + "X", aBook.getEdition(), aBook.getIsbn(), aBook.getYearOfPublication()));
		verify(bookRepository, times(0)).save(any(Book.class));
	}
