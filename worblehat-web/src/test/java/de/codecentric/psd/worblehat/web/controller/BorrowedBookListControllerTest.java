...
class BorrowedBookListControllerTest {
  ...
  @InjectMocks private BorrowedBookListController controllerUnderTest;
  ...
  void shouldReturnOneBorrowedBook() {
    ...
    when(bookServiceMock.findAllBorrowingsByEmailAddress(emailAddress))
        .thenReturn(List.of(borrowingWith(emailAddress)));

    var borrowedBookList = new ModelMap();
    String page =
        controllerUnderTest.findBorrowedBooksByEmailAddress(
            emailAddress(emailAddress),
            new MapBindingResult(new HashMap<>(), ""),
            borrowedBookList);
